...
Release Please pull requests get an additional `Release PR Validation` workflow. GitHub can suppress ordinary pull-request workflows when a workflow token creates or updates the release branch, so this narrow `pull_request_target` workflow validates only same-repository `github-actions[bot]` release-please branches. It checks out the pull-request head SHA with `persist-credentials: false`, runs the same Python 3.11, 3.12, 3.13, and 3.14 test matrix, and keeps write permissions out of the job.
